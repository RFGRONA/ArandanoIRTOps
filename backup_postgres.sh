...
#              bucket de MinIO.
# Versión de Repositorio: 1.2 (Corregido con ShellCheck)
# ==============================================================================
...
if docker exec -u postgres "${DB_CONTAINER}" pg_dump -U "${DB_USER}" -d "${DB_NAME}" | gzip > "${FULL_PATH}"; then
...
if mc cp "${FULL_PATH}" "${MINIO_ALIAS}/${MINIO_BUCKET}/"; then
